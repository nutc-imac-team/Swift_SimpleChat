...
class Message {
    ...
    func downloadLastMessage(forLocation: String, completion: @escaping (Bool) -> ()) {
        if let currentUserID = Auth.auth().currentUser?.uid {
    ...
    class func send(message: Message, toID: String, completion: @escaping (Bool) -> Swift.Void)  {
        if let currentUserID = Auth.auth().currentUser?.uid {
            switch message.type {
            case .photo:
                let imageData = UIImageJPEGRepresentation((message.content as! UIImage), 0.1)
